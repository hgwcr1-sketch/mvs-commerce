...
class CashSessionMailNotificationTest extends TestCase
{
    ...
    public function test_configured_recipients_are_company_scoped_and_close_delivery_is_idempotent(): void
    {
        Queue::fake();
        Mail::fake();
        [$company, $branch, $user, $register, $settings] = $this->context(['caja.administrar', 'caja.cerrar']);
        [$other, , , , $otherSettings] = $this->context();
        $otherSettings->update(['closure_email_recipients' => ['other@example.com']]);
        $this->actingAs($user)->withSession(['active_company_id' => $company->id, 'active_branch_id' => $branch->id]);
        $this->get(route('settings.cash.edit'))->assertOk()->assertSee('closure_email_recipients[]', false)->assertSee('Agregar correo');
        $this->put(route('settings.cash.update'), [
            'company_id' => $other->id, 'session_mode' => 'individual', 'difference_tolerance' => 0,
            'require_open_session' => true,
            'closure_email_recipients' => [' ADMIN@EXAMPLE.COM ', 'admin@example.com', 'finance@example.com'],
        ])->assertSessionHasNoErrors();
        $this->assertSame(['admin@example.com', 'finance@example.com'], $settings->fresh()->closure_email_recipients);
        $this->assertSame(['other@example.com'], $otherSettings->fresh()->closure_email_recipients);
        ...
        $payload = $this->closingPayload($company, (string) Str::uuid(), 1000);
        $service->submit($user, $company->id, $branch->id, $session->id, $payload);
        $service->submit($user, $company->id, $branch->id, $session->id, $payload);
        $notification = CashSessionMailNotification::sole();
        ...
        Mail::assertSent(CashSessionClosedMail::class, 2);
        Mail::assertNotSent(CashSessionClosedMail::class, fn ($mail) => $mail->hasTo('other@example.com') || $mail->hasTo($user->email));
    }

    public function test_removing_all_recipients_keeps_closure_successful_without_delivery(): void
    {
        Queue::fake();
        Mail::fake();
        [$company, $branch, $user, $register, $settings] = $this->context(['caja.administrar', 'caja.cerrar']);
        $settings->update(['closure_email_recipients' => ['old@example.com']]);
        $this->actingAs($user)->withSession(['active_company_id' => $company->id, 'active_branch_id' => $branch->id])
            ->put(route('settings.cash.update'), ['session_mode' => 'individual', 'difference_tolerance' => 0, 'require_open_session' => true, 'closure_email_recipients' => []])
            ->assertSessionHasNoErrors();
        $this->assertNull($settings->fresh()->closure_email_recipients);
        $session = $this->rawSession($company, $branch, $user, $register);
        $service = app(CashClosingService::class);
        $service->start($user, $company->id, $branch->id, $session->id, (string) Str::uuid());
        $service->submit($user, $company->id, $branch->id, $session->id, $this->closingPayload($company, (string) Str::uuid(), 1000));
        $notification = CashSessionMailNotification::sole();
        (new SendCashSessionMailNotification($notification->id))->handle();
        $this->assertSame('closed', $session->fresh()->status);
        $this->assertSame('skipped', $notification->status);
        $this->assertSame([], $notification->recipients);
        Queue::assertNothingPushed();
        Mail::assertNothingSent();
    }

    public function test_cash_sender_and_support_reply_to_do_not_change_system_sender(): void
    {
        config(['mail.from.address' => 'no-reply@example.com', 'mail.notifications.from.address' => 'notifications@example.com', 'mail.notifications.from.name' => 'Caja', 'mail.reply_to.address' => 'support@example.com']);
        [$company, $branch, $user, $register] = $this->context();
        $session = $this->rawSession($company, $branch, $user, $register);
        foreach ([new CashSessionOpenedMail($session), new CashSessionClosedMail($session)] as $mail) {
            $envelope = $mail->envelope();
            $this->assertSame('notifications@example.com', $envelope->from->address);
            $this->assertSame('support@example.com', $envelope->replyTo[0]->address);
        }
        $this->assertSame('no-reply@example.com', config('mail.from.address'));
        $authMail = (new ResetPassword('test-token'))->toMail($user);
        $this->assertEmpty($authMail->from); // Auth inherits mail.from; Caja does not override it globally.
    }

    public function test_foreign_company_mail_settings_are_rejected(): void
    {
        Queue::fake();
        [$company, $branch, $user, $register] = $this->context();
        [, , , , $otherSettings] = $this->context();
        $session = $this->rawSession($company, $branch, $user, $register);
        $this->expectException(\InvalidArgumentException::class);
        app(CashSessionMailNotificationService::class)->create($session, CashSessionMailNotification::TYPE_CLOSED, $otherSettings);
    }

    public function test_closed_mail_retry_skips_delivered_recipient_and_duplicate_job(): void
    {
        Queue::fake();
        Mail::fake();
        [$company, $branch, $user, $register] = $this->context();
        $session = $this->rawSession($company, $branch, $user, $register, ['status' => 'closed', 'open_guard' => null, 'closed_by' => $user->id, 'closed_at' => now()]);
        $notification = CashSessionMailNotification::create(['company_id' => $company->id, 'cash_session_id' => $session->id, 'notification_type' => 'closed', 'recipients' => ['one@example.com', 'two@example.com'], 'delivered_recipients' => ['one@example.com'], 'status' => 'failed', 'attempts' => 1, 'available_at' => now()->subSecond()]);
        $job = new SendCashSessionMailNotification($notification->id);
        $job->handle();
        $job->handle();
        Mail::assertSent(CashSessionClosedMail::class, 1);
        Mail::assertSent(CashSessionClosedMail::class, fn ($mail) => $mail->hasTo('two@example.com') && ! $mail->hasTo('one@example.com'));
        $this->assertSame('closed', $session->fresh()->status);
        $this->assertSame('sent', $notification->fresh()->status);
    ...
    {
        Queue::fake();
        [$company, $branch, $user, $register, $settings] = $this->context();
    ...
    {
        Queue::fake();
        [$company, $branch, $user, $register, $settings] = $this->context(['caja.cerrar']);
        $settings->update(['closure_email_recipients' => ['admin@example.com']]);
        $session = $this->rawSession($company, $branch, $user, $register);
        $service = app(CashClosingService::class);
    ...
    {
        Queue::fake();
        [$company, $branch, $user, $register, $settings] = $this->context(['caja.cerrar', 'caja.autorizar_diferencia']);
    ...
    public function test_transport_failure_is_sanitized_retriable_and_does_not_change_session(): void
    {
        ...
        $delivered = Mockery::mock();
        $delivered->shouldReceive('send')->once();
        $pending = Mockery::mock();
        $pending->shouldReceive('send')->once()->andThrow(new RuntimeException('SMTP failed for secret@example.com'));
    ...
    public function test_recovery_command_dispatches_only_available_company_isolated_rows(): void
    {
        Queue::fake();
        [$company, $branch, $user, $register] = $this->context();
        [$other, $otherBranch, $otherUser, $otherRegister] = $this->context();
        $session = $this->rawSession($company, $branch, $user, $register);
        $otherSession = $this->rawSession($other, $otherBranch, $otherUser, $otherRegister);
        ...
        $this->assertSame($company->id, $available->company_id);
        $this->assertSame($other->id, $future->company_id);
    }
    ...
    public function test_mailables_render_timezone_snapshots_and_conditional_usd_without_web_leakage(): void
    {
        ...
        $this->assertStringContainsString('14/08/2026 12:00:00', $openingHtml);
        $this->assertStringContainsString('Fondo inicial USD', $openingHtml);
        ...
        $denomination = CashDenomination::forCompany($company->id)->active()->first();
        ...
        $this->assertStringContainsString('Diferencia general', $closingHtml);
        $this->assertStringContainsString('-10,00', $closingHtml);
        $this->assertStringContainsString($branch->name, (new CashSessionClosedMail($session->fresh()))->envelope()->subject);
        $this->assertStringContainsString($register->name, $closingHtml);
        $this->assertStringContainsString(html_entity_decode($user->name, ENT_QUOTES | ENT_HTML5), $closingHtml);
        $this->assertStringContainsString('Nombre histórico', $closingHtml);
        $this->assertStringContainsString('old-code', $closingHtml);
        $this->assertStringContainsString('1.234', $closingHtml);
        $this->assertStringContainsString('Nota auditada', $closingHtml);
    ...
    private function context(array $permissions = []): array
    {
        ...
        $settings = app(CompanyCashSettingsProvisioner::class)->provision($company);
        app(CashDenominationProvisioner::class)->provision($company);
        $user = User::factory()->create();
        $role = Role::create(['company_id' => $company->id, 'name' => 'Rol '.Str::random(5), 'is_active' => true]);
    ...
    {
        $denominations = CashDenomination::forCompany($company->id)->forCurrency('CRC')->active()->orderByDesc('value')->get();
        $remaining = $total;
        $counts = [];
        foreach ($denominations as $denomination) {
            $counts[$denomination->id] = intdiv($remaining, (int) $denomination->value);
            $remaining %= (int) $denomination->value;
        }
