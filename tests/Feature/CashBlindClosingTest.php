...
class CashBlindClosingTest extends TestCase
{
    ...
    public function test_custom_cash_effect_is_snapshotted_for_totals_even_after_method_configuration_changes(): void
    {
        [$company,$branch,$user,,$session] = $this->context();
        $custom = $this->method($company, 'custom_cash', 'Efectivo personalizado', true, true);
        $custom->update(['type' => PaymentMethod::TYPE_OTHER]);
        $card = $this->method($company, 'card', 'Tarjeta', true);
        $this->payment($company, $branch, $user, $session, $custom, 100);
        $this->payment($company, $branch, $user, $session, $card, 300);
        $this->start($user, $company, $branch, $session);
        $this->submit($user, $company, $branch, $session, $this->payload($company, $session, 1100, [$custom->id => 100, $card->id => 300]))->assertSessionHasNoErrors();
        $this->assertTrue($session->paymentReconciliations()->where('payment_method_id', $custom->id)->firstOrFail()->affects_cash_snapshot);
        $custom->update(['affects_cash' => false, 'name' => 'Nombre cambiado']);
        $summary = app(CashClosingSummaryService::class)->summarize($session->fresh());
        $this->assertSame([
            'expected' => '1400.0000',
            'reported' => '1400.0000',
            'difference' => '0.0000',
            'completed_sales_count' => 2,
            'account_receivable_payments_count' => 0,
            'layaway_payments_count' => 0,
            'account_payable_payments_count' => 0,
            'documents_count' => 2,
        ], $summary);
    }

    public function test_employee_remains_blind_when_legacy_setting_is_disabled_and_inputs_start_empty(): void
    {
        [$company,$branch,$user,,$session] = $this->context();
        $session->update(['blind_closing_snapshot' => false]);
        $method = $this->method($company, 'sinpe', 'SINPE', true);
        $this->payment($company, $branch, $user, $session, $method, 123456);
        $this->start($user, $company, $branch, $session);
        $response = $this->actingAs($user)->withSession($this->ctx($company, $branch))->get(route('cash.closing.create', $session));
        $response->assertOk()->assertViewHas('blind', true)->assertViewHas('expectedCash', null)
            ->assertViewHas('expectedMethods', fn ($v) => $v->isEmpty())
            ->assertViewHas('expectedBreakdown', fn ($v) => $v->isEmpty())
            ->assertDontSee('Total esperado')->assertDontSee('123456')->assertDontSee('123.456');
        $response->assertSee('required value=""', false);
        $source = file_get_contents(resource_path('views/cash/closing/create.blade.php'));
        $this->assertStringContainsString('old("denominations.$d->id","")', $source);
        $this->assertStringContainsString('old("payments.$m->id.reported_amount","")', $source);
    }

    public function test_administrative_result_has_audit_and_general_total_without_double_counting_cash(): void
    {
        [$company,$branch,$user,,$session] = $this->context();
        $cash = $this->method($company, 'cash', 'Efectivo', true, true);
        $card = $this->method($company, 'card', 'Tarjeta', true);
        $sinpe = $this->method($company, 'sinpe', 'SINPE', true);
        $other = $this->method($company, 'custom', 'Medio personalizado', true);
        foreach ([[$cash, 500], [$card, 200], [$sinpe, 300], [$other, 400]] as [$method,$amount]) {
            $this->payment($company, $branch, $user, $session, $method, $amount);
        }
        $this->start($user, $company, $branch, $session);
        $this->submit($user, $company, $branch, $session, $this->payload($company, $session, 1495, [$cash->id => 500, $card->id => 190, $sinpe->id => 303, $other->id => 400]))->assertSessionHasNoErrors();
        $admin = $this->user($company, $branch, ['caja.ver', 'caja.administrar']);
        $this->actingAs($admin)->withSession($this->ctx($company, $branch))->get(route('cash.closing.show', $session))->assertOk()
            ->assertSee('Eventos y autorizaciones')->assertSee('Medio personalizado')->assertSee('Abierta por')->assertSee('Cerrada por')
            ->assertViewHas('closingSummary', [
                'expected' => '2400.0000',
                'reported' => '2388.0000',
                'difference' => '-12.0000',
        ...
        $this->actingAs($user)->get(route('cash.closing.show', $session))->assertOk()->assertDontSee('Total esperado')->assertDontSee('Diferencia general');
    }
    ...
    public function test_start_is_idempotent_freezes_session_and_cancel_reopens_before_submission(): void
    {
        ...
        $this->assertSame(CashSession::OPEN_GUARD, $session->fresh()->open_guard);
        ...
        $this->assertSame(1, $session->events()->where('event_type', CashSessionEvent::TYPE_CLOSING_CANCELLED)->count());
    }
    ...
    public function test_closing_form_has_denominations_dynamic_methods_and_source_breakdown(): void
    {
        ...
        $cash = $this->method($company, 'cash', 'Efectivo', true, true);
        $card = $this->method($company, 'card', 'Tarjeta', true);
        $paypal = $this->method($company, 'paypal', 'PayPal', false);
        ...
        $response->assertOk()->assertSee('Cierre ciego activo')->assertSee('Efectivo')->assertSee('Tarjeta')->assertSee('PayPal')->assertSee('Billetes')->assertSee('Monedas')->assertSee('Volver')
            ->assertDontSee('Total esperado')->assertDontSee('₡777')->assertDontSee('difference')->assertDontSee('tolerance')
    ...
    public function test_distinct_declared_quantities_are_saved_by_denomination_id_and_total_fifty_thousand(): void
    {
        ...
        $this->assertNotNull($twentyThousand);
        $this->assertNotNull($tenThousand);
        $this->assertNotNull($fiveThousand);
        ...
        foreach ($denominations as $denomination) {
            $expectedQuantity = match ((int) $denomination->value) {
                20000 => 2, 10000 => 1, default => 0
            };
    ...
    {
        [$company, $branch, $user, , $session] = $this->context();
        $session->update(['opening_amount' => 1000]);
        $cash = $this->method($company, 'cash', 'Efectivo', true, true);
        $card = $this->method($company, 'card', 'Tarjeta', true);
        $this->payment($company, $branch, $user, $session, $cash, 500, 2000, 1500);
        $this->payment($company, $branch, $user, $session, $card, 300);
        $this->movement($session, $user, CashMovement::TYPE_ENTRY, CashMovement::DIRECTION_IN, 100);
        $this->movement($session, $user, CashMovement::TYPE_EXIT, CashMovement::DIRECTION_OUT, 50);
        $this->start($user, $company, $branch, $session);
        $payload = $this->payload($company, $session, 1550, [$cash->id => 500, $card->id => 300]);
        $this->submit($user, $company, $branch, $session, $payload)->assertRedirect(route('cash.closing.show', $session));
        $closed = $session->fresh();
        $this->assertSame(CashSession::STATUS_CLOSED, $closed->status);
        $this->assertNull($closed->open_guard);
        $this->assertSame('1550.0000', $closed->expected_cash);
        $this->assertSame('1550.0000', $closed->counted_cash);
        $this->assertSame('0.0000', $closed->difference_amount);
        $this->assertSame(11, $closed->countDetails()->closing()->count());
        $this->assertSame(2, $closed->paymentReconciliations()->count());
        ...
        $this->assertSame('Tarjeta', $reconciliations->get($card->id)->payment_method_name_snapshot);
        $this->assertSame('300.0000', $reconciliations->get($card->id)->expected_amount);
    }
    ...
    public function test_payment_origins_are_visible_and_persisted_without_changing_method_totals(): void
    {
        ...
        foreach ([[$cash, 300], [$card, 400]] as [$method,$amount]) {
            LayawayPayment::create(['layaway_id' => $layaway->id, 'company_id' => $company->id, 'branch_id' => $branch->id, 'user_id' => $user->id, 'cash_session_id' => $session->id, 'payment_method_id' => $method->id, 'amount' => $amount, 'affects_cash_snapshot' => $method->affects_cash, 'cash_effect_amount' => $method->affects_cash ? $amount : 0, 'paid_at' => now()]);
        }

        $service = app(CashPaymentExpectedAmountService::class);
        ...
        $this->actingAs($user)->withSession($this->ctx($company, $branch))->get(route('cash.closing.create', $session))
            ->assertOk()->assertDontSee('Total esperado')->assertDontSee('₡600')
            ->assertViewHas('expectedMethods', fn ($values) => $values->isEmpty())
            ->assertViewHas('expectedBreakdown', fn ($values) => $values->isEmpty());
        ...
        $this->assertSame('100.0000', $rows[$cash->id]->sales_amount);
        $this->assertSame('200.0000', $rows[$cash->id]->receivables_amount);
        $this->assertSame('300.0000', $rows[$cash->id]->layaways_amount);
        $this->assertSame('600.0000', $rows[$cash->id]->expected_amount);
        $this->assertSame('0.0000', $rows[$card->id]->sales_amount);
        $this->assertSame('0.0000', $rows[$card->id]->receivables_amount);
        $this->assertSame('400.0000', $rows[$card->id]->layaways_amount);
        $this->assertSame('400.0000', $rows[$card->id]->expected_amount);

        AccountReceivablePayment::query()->update(['amount' => 999, 'cash_effect_amount' => 999]);
        LayawayPayment::query()->update(['amount' => 999, 'cash_effect_amount' => 999]);
        ...
        // El conteo comercial usa las cuatro fuentes, sin contar pagos de venta ni movimientos.
        $supplier = Supplier::create(['company_id' => $company->id, 'supplier_type' => 'company', 'name' => 'Proveedor documentos', 'is_active' => true]);
        $purchase = Purchase::create(['company_id' => $company->id, 'branch_id' => $branch->id, 'supplier_id' => $supplier->id, 'user_id' => $user->id, 'number' => 'CP-DOC', 'purchase_date' => today(), 'payment_type' => 'credit', 'due_date' => today()->addMonth(), 'total' => 1000, 'status' => 'posted']);
        $payable = AccountPayable::create(['company_id' => $company->id, 'branch_id' => $branch->id, 'supplier_id' => $supplier->id, 'purchase_id' => $purchase->id, 'original_amount' => 1000, 'paid_amount' => 0, 'balance_due' => 1000, 'issue_date' => today(), 'due_date' => today()->addMonth(), 'status' => 'pending', 'created_by' => $user->id]);
        AccountPayablePayment::create(['account_payable_id' => $payable->id, 'company_id' => $company->id, 'branch_id' => $branch->id, 'supplier_id' => $supplier->id, 'user_id' => $user->id, 'payment_method_id' => $card->id, 'cash_session_id' => $session->id, 'amount' => 100, 'affects_cash_snapshot' => false, 'cash_effect_amount' => 0, 'paid_at' => now()]);
        $breakdown = ['completed_sales_count' => 2, 'account_receivable_payments_count' => 1, 'layaway_payments_count' => 2, 'account_payable_payments_count' => 1, 'documents_count' => 6];
        $this->assertSame($breakdown, $session->documentsBreakdown());
        $this->movement($session, $user, CashMovement::TYPE_ENTRY, CashMovement::DIRECTION_IN, 10);
        $payment = SalePayment::where('cash_session_id', $session->id)->firstOrFail()->replicate();
        $payment->save();
        $this->assertSame($breakdown, $session->fresh()->documentsBreakdown());
        $this->assertSame(6, $session->fresh()->documents_count);
        $summary = app(CashClosingSummaryService::class)->summarize($session->fresh());
        foreach ($breakdown as $key => $value) {
            $this->assertSame($value, $summary[$key]);
        }
        $mail = new CashSessionClosedMail($session->fresh());
        $this->assertSame(6, $mail->content()->with['closingSummary']['documents_count']);
        [, , , , $otherSession] = $this->context('Documentos de otra empresa');
        $this->assertSame(0, $otherSession->documents_count);
    }
    ...
    public function test_surplus_and_shortage_are_stored_without_compensation(): void
    {
        foreach ([5 => '5.0000', -5 => '-5.0000'] as $delta => $expected) {
            [$company, $branch, $user, , $session] = $this->context('Diff'.$delta);
            $this->start($user, $company, $branch, $session);
    ...
    public function test_voided_payments_and_non_completed_sales_are_excluded(): void
    {
        ...
        $voidedPayment = SalePayment::latest('id')->first();
        $voidedPayment->update(['status' => SalePayment::STATUS_VOIDED]);
        $this->payment($company, $branch, $user, $session, $card, 200);
        Sale::latest('id')->first()->update(['status' => Sale::STATUS_VOIDED]);
        $this->payment($company, $branch, $user, $session, $card, 300);
        $expected = app(CashPaymentExpectedAmountService::class)->expectedAmounts($session);
    ...
    {
        [$company, $branch, $user, , $session] = $this->context();
        CompanyCashSetting::where('company_id', $company->id)->update(['require_difference_authorization' => true]);
        $session->update(['tolerance_snapshot' => 5]);
        $this->start($user, $company, $branch, $session);
        $this->submit($user, $company, $branch, $session, $this->payload($company, $session, 995))->assertRedirect();
        $this->assertSame(CashSession::STATUS_CLOSED, $session->fresh()->status);
        $this->assertNull($session->fresh()->difference_authorized_by);
    }
    ...
    public function test_each_difference_is_compared_to_tolerance_and_requires_specific_authorization(): void
    {
        ...
        $this->assertSame(CashSession::STATUS_CLOSED, $session->fresh()->status);
        $this->assertSame($authorizer->id, $session->fresh()->difference_authorized_by);
    ...
    public function test_confirmation_is_idempotent_and_cannot_be_cancelled_or_reopened_after_submission(): void
    {
        [$company, $branch, $user, , $session] = $this->context();
        $this->start($user, $company, $branch, $session);
        $payload = $this->payload($company, $session, 1000);
        $this->submit($user, $company, $branch, $session, $payload)->assertRedirect();
        $this->submit($user, $company, $branch, $session, $payload)->assertRedirect();
        $this->assertSame(11, CashCountDetail::count());
        $this->assertSame(1, $session->events()->where('event_type', CashSessionEvent::TYPE_CLOSED)->count());
    ...
    public function test_invalid_incomplete_extra_and_inactive_denominations_are_rejected(): void
    {
        ...
        $this->submit($user, $company, $branch, $session, $payload)->assertSessionHasErrors('denominations');
        $payload = $this->payload($company, $session, 1000);
        $payload['denominations'][999999] = 0;
        $this->submit($user, $company, $branch, $session, $payload)->assertSessionHasErrors('denominations');
        $payload = $this->payload($company, $session, 1000);
        CashDenomination::forCompany($company->id)->first()->update(['is_active' => false]);
        $this->submit($user, $company, $branch, $session, $payload)->assertSessionHasErrors('denominations');
    ...
    {
        [$company, $branch, $user, , $session] = $this->context();
        $method = $this->method($company, 'card', 'Tarjeta', true);
        $this->start($user, $company, $branch, $session);
        CashPaymentReconciliation::creating(fn () => throw new \RuntimeException('reconciliation failure'));
        $this->withoutExceptionHandling();
        try {
            $this->submit($user, $company, $branch, $session, $this->payload($company, $session, 1000, [$method->id => 0]));
            $this->fail('No falló.');
        } catch (\RuntimeException $e) {
            $this->assertSame('reconciliation failure', $e->getMessage());
        }
        $this->assertSame(0, CashCountDetail::count());
        $this->assertSame(0, CashPaymentReconciliation::count());
        $this->assertNull($session->fresh()->closing_submitted_at);
    }
    ...
    public function test_closed_event_failure_rolls_back_session_counts_and_reconciliations(): void
    {
        [$company,$branch,$user,,$session] = $this->context();
        $this->start($user, $company, $branch, $session);
        CashSessionEvent::creating(function (CashSessionEvent $event) {
            if ($event->event_type === CashSessionEvent::TYPE_CLOSED) {
                throw new \RuntimeException('event failure');
            }
        });
        $this->withoutExceptionHandling();
        try {
            $this->submit($user, $company, $branch, $session, $this->payload($company, $session, 1000));
            $this->fail('No falló.');
        } catch (\RuntimeException $e) {
    ...
    public function test_individual_and_shared_isolation_permissions_are_enforced(): void
    {
        ...
        $other = $this->user($company, $branch, ['caja.cerrar']);
        ...
        $none = $this->user($company, $branch, []);
        $this->start($none, $company, $branch, $session)->assertForbidden();
    }
    ...
    {
        foreach (['cash.counts.edit', 'cash.counts.update', 'cash.counts.destroy', 'cash.reconciliations.edit', 'cash.reconciliations.update', 'cash.reconciliations.destroy'] as $name) {
    ...
    private function context(string $name = 'Empresa'): array
    {
        ...
        $user = $this->user($company, $branch, ['caja.cerrar', 'caja.ver']);
        $register = CashRegister::create(['company_id' => $company->id, 'branch_id' => $branch->id, 'code' => 'C'.uniqid(), 'name' => 'Caja', 'is_active' => true]);
        $session = CashSession::create(['company_id' => $company->id, 'branch_id' => $branch->id, 'cash_register_id' => $register->id, 'session_number' => 'CAJA-'.$company->id, 'opened_by' => $user->id, 'status' => CashSession::STATUS_OPEN, 'open_guard' => CashSession::OPEN_GUARD, 'opening_amount' => 1000, 'tolerance_snapshot' => 0, 'blind_closing_snapshot' => true, 'opened_at' => now()]);
    ...
    {
        return PaymentMethod::create(['company_id' => $company->id, 'code' => $code, 'name' => $name, 'type' => $cash ? PaymentMethod::TYPE_CASH : ($code === 'sinpe' ? PaymentMethod::TYPE_SINPE : PaymentMethod::TYPE_OTHER), 'is_active' => $active, 'affects_cash' => $cash, 'sort_order' => 1]);
    }

    private function payment(Company $company, Branch $branch, User $user, CashSession $session, PaymentMethod $method, float $amount, float $received = 0, float $change = 0): void
    {
        $sale = Sale::create(['company_id' => $company->id, 'branch_id' => $branch->id, 'user_id' => $user->id, 'cash_session_id' => $session->id, 'sale_number' => 'POS-'.Str::random(6), 'checkout_token' => Str::uuid(), 'request_fingerprint' => hash('sha256', Str::random()), 'status' => Sale::STATUS_COMPLETED, 'completed_at' => now()]);
        SalePayment::create(['sale_id' => $sale->id, 'cash_session_id' => $session->id, 'payment_method_id' => $method->id, 'affects_cash_snapshot' => $method->affects_cash, 'created_by' => $user->id, 'amount' => $amount, 'received_amount' => $received ?: $amount, 'change_amount' => $change, 'cash_effect_amount' => $method->affects_cash ? $amount : 0, 'status' => SalePayment::STATUS_COMPLETED]);
    }

    private function movement(CashSession $session, User $user, string $type, string $direction, float $amount): void
    {
        CashMovement::create(['company_id' => $session->company_id, 'branch_id' => $session->branch_id, 'cash_register_id' => $session->cash_register_id, 'cash_session_id' => $session->id, 'type' => $type, 'direction' => $direction, 'amount' => $amount, 'concept' => $type, 'reason' => 'Prueba', 'created_by' => $user->id, 'occurred_at' => now()]);
    }

    private function start(User $user, Company $company, Branch $branch, CashSession $session, ?string $token = null)
    {
        return $this->actingAs($user)->withSession($this->ctx($company, $branch))->post(route('cash.closing.start', $session), ['request_token' => $token ?? (string) Str::uuid()]);
    }

    private function submit(User $user, Company $company, Branch $branch, CashSession $session, array $payload)
    {
        return $this->actingAs($user)->withSession($this->ctx($company, $branch))->post(route('cash.closing.submit', $session), $payload);
    }
    ...
    private function payload(Company $company, CashSession $session, int $cashTotal, array $reports = []): array
    {
        ...
        $counts = $denominations->mapWithKeys(fn ($d) => [$d->id => 0])->all();
        $remaining = $cashTotal;
        foreach ($denominations as $d) {
            $counts[$d->id] = intdiv($remaining, (int) $d->value);
            $remaining %= (int) $d->value;
        }$methods = app(CashPaymentExpectedAmountService::class)->methods($session);
        $payments = $methods->mapWithKeys(fn ($m) => [$m->id => ['reported_amount' => $reports[$m->id] ?? 0, 'reference' => null, 'notes' => null]])->all();

        return ['request_token' => (string) Str::uuid(), 'denominations' => $counts, 'payments' => $payments, 'closing_notes' => null];
    }
}
