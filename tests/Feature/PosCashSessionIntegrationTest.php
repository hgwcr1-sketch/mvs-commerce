...
class PosCashSessionIntegrationTest extends TestCase
{
    ...
    private function product(Company $c,bool $tracked=false): Product{$id=uniqid();$cat=ProductCategory::create(['company_id'=>$c->id,'name'=>'C'.$id,'slug'=>'c'.$id,'is_active'=>true]);$unit=Unit::create(['company_id'=>$c->id,'name'=>'Unidad','abbreviation'=>'U','slug'=>'u'.$id,'is_active'=>true]);$product=Product::create(['company_id'=>$c->id,'category_id'=>$cat->id,'unit_id'=>$unit->id,'name'=>'Producto','internal_code'=>'P'.$id,'cost'=>500,'sale_price'=>1000,'tax_rate'=>0,'track_inventory'=>$tracked,'is_active'=>true]);$product->fiscal_profile_id=\App\Models\FiscalProfile::query()->where('tax_code','01')->where('tax_rate_code','10')->value('id');$product->save();return $product;}
