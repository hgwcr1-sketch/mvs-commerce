...
## Estado actual de Portal de Clientes (P01–P07) — reconciliado con Excel único

Fuente oficial: `docs/CRONOGRAMA_PRODUCCION.md` (P01–P50) y referencia visual `docs/Cronograma_Unico_Portal_Correcciones_MVS_Commerce_28-08-2026.xlsx`. Reemplaza cronogramas anteriores; **P01–P07 COMPLETADOS**, P08 es el siguiente bloque, P09A–P09D conservan esos IDs exactos, migración P31–P40 después de los bloques existentes sin reutilizar IDs.
...
- Evidencia P01–P07: `LoyaltyPortalSelfRegistrationTest` **11/11, 52 aserciones, 0 fallos** + `LoyaltyPortalClientAccessTest` **11/11, 55 aserciones, 0 fallos** (incl. `test_temporary_password_requires_change_on_first_login` y `test_portal_password_is_not_generic` con `must_change_password`). `LoyaltyCustomerPortal` 13/13.
