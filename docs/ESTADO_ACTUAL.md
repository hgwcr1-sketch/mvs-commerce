...
## Estado actual de Portal de Clientes (P01–P09B) — reconciliado con Excel único

Fuente oficial: `docs/CRONOGRAMA_PRODUCCION.md` (P01–P50) y referencia visual `docs/Cronograma_Unico_Portal_Correcciones_MVS_Commerce_28-08-2026.xlsx`. Reemplaza cronogramas anteriores; **P01–P09B COMPLETADOS**, P09C es el siguiente bloque, P09D conserva su ID exacto, migración P31–P40 después de los bloques existentes sin reutilizar IDs.
...
- Evidencia P01–P09B: `LoyaltyPortalSelfRegistrationTest` **11/11, 52 aserciones** + `LoyaltyPortalClientAccessTest` **11/11, 55 aserciones** + `LoyaltyPortalDeliveryTest` **7/7, 51 aserciones** + `LoyaltyPortalCentralTest` **4/4, 17 aserciones** + `CustomerPublicCodeTest` **5/5, 23 aserciones** + `CustomerQrBarcodeTest` **4/4, 16 aserciones, 0 fallos** (`qrSvg`+`barcodeSvg` locales, `clientes/show` sin leak). `LoyaltyCustomerPortal` 13/13.
