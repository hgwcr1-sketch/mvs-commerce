...
- **P37–P40 — COMPLETADOS ADELANTADAMENTE por autorización expresa.** P37 importa puntos otorgados, utilizados y saldo conciliado por cliente mediante plantilla simple de 4 columnas (`NOMBRE`, `PUNTOS OTORGADOS`, `PUNTOS UTILIZADOS`, `SALDO`), cliente único por nombre normalizado y empresa; movimientos vía `LoyaltyAccountService`; lote derivado del contenido, idempotencia, rollback y bloqueo de cuentas operativas; P38 exporta ZIP con manifiesto SHA-256; P39 aporta preview, errores CSV, revalidación, idempotencia y rollback; P40 documenta/prueba conciliación SQLite→PostgreSQL sin ejecutarla ni tocar producción. Evidencia focal: 7 tests, 61 aserciones; regresión relacionada: 62 tests, 488 aserciones.
