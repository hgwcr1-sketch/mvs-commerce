...
| P37 | Importar saldos y movimientos históricos de puntos | **COMPLETADO ADELANTADAMENTE (AUTORIZADO)** | Snapshot por cliente + Kardex histórico sin reaplicar puntos; aislamiento, bloqueo de cuentas operativas, lote único, idempotencia y rollback. `LoyaltyMigrationP37Test` 6/6. |
...
- **P31–P36: COMPLETADOS ADELANTADAMENTE por autorización expresa** – infraestructura común identificada/reutilizada; Clientes, Productos, Ventas históricas e Inventario/Kardex quedan migrables con preview, validación, idempotencia, exportación y rollback. P36 separa saldo inicial (sí fija stock) de Kardex histórico (no altera stock actual). Esto no cambia el orden pendiente: P25 sigue siendo el siguiente bloque oficial; P37 no se implementó.
