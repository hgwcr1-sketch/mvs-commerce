...
- P37 importa saldos de fidelización mediante plantilla simple aprobada de 4 columnas; puede aprovechar identificación, teléfono o email opcionales ya presentes en el origen para desambiguar de forma exacta (sin fuzzy matching). Su `source_key` se deriva únicamente de filas fuente normalizadas y conserva compatibilidad con la fórmula histórica de cuatro columnas. Consolida por cliente históricos conciliados sumando otorgados/utilizados y snapshots legacy solo cuando el saldo final coincide; mezclas o snapshots incompatibles quedan pendientes. La confirmación importa únicamente clientes válidos, registra pendientes y motivos, conserva resoluciones manuales por empresa+lote+fila+nombre y mantiene idempotencia, rollback y bloqueo de cuentas operativas.
...
- política ampliada localmente el 07/09/2026: compra calificadora tiene prioridad; en su ausencia, un único movimiento P37 `legacy_initial_balance` válido aporta su `effective_at`. Sin ambas referencias, o con saldo cero, no se generan vencimientos. Portal y proceso automático comparten `LoyaltyExpirationPolicyService`; no hay backfill, cambios históricos ni migraciones. No usar `last_activity_at` ni convertir P37 en compra. Ver decisión de vencimiento P37 en `DECISIONES.md`. Despliegue pendiente de autorización;
- fixtures corregidos con autorización: 24 valores `national` y uno `physical` reemplazados por `01` en seis archivos de tests, sin cambiar schema ni lógica. Validación final de la suite nueva: SQLite 15 aprobadas / 69 aserciones y concurrencia omitida por requerir PostgreSQL; PostgreSQL aislado 16 aprobadas / 77 aserciones, incluida concurrencia real y flujo P37 → portal → compra → expiración completa. Revisión independiente atribuyó los fallos restantes a deuda ajena y autorizó el commit limitado; no deploy. Resultados de regresión amplia y límites en `ESTADO_ACTUAL.md`;
