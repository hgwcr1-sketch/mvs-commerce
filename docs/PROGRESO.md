...
- **Evidencia P01–P07:** `LoyaltyPortalSelfRegistrationTest` **11/11, 52 aserciones, 0 fallos** + `LoyaltyPortalClientAccessTest` **11/11, 55 aserciones, 0 fallos** (incl. `test_temporary_password_requires_change_on_first_login` y `test_portal_password_is_not_generic`). `LoyaltyCustomerPortal` 13/13, 89 aserciones.
- **P08 y siguientes: PENDIENTES.** Siguiente **P08** – Mostrar URL, usuario, Copiar acceso, WhatsApp y QR al cajero; no P09A identidad pública única, no P09B QR+Code128, no P09C lectura POS, no P09D PIN/QR temporal.
