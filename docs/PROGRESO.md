...
- **Evidencia P01–P08:** `LoyaltyPortalSelfRegistrationTest` **11/11, 52 aserciones** + `LoyaltyPortalClientAccessTest` **11/11, 55 aserciones** + `LoyaltyPortalDeliveryTest` **7/7, 51 aserciones, 0 fallos**. `LoyaltyCustomerPortal` 13/13, 89 aserciones.
- **P09 y siguientes: PENDIENTES.** Siguiente **P09** – Pantalla central con URL general, QR, copiar y vista previa (Fidelización → Portal de Clientes); no P09A identidad pública única, no P09B QR+Code128, no P09C lectura POS, no P09D PIN/QR temporal.
