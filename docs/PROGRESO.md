...
- **Evidencia P01–P09:** `LoyaltyPortalSelfRegistrationTest` **11/11, 52 aserciones** + `LoyaltyPortalClientAccessTest` **11/11, 55 aserciones** + `LoyaltyPortalDeliveryTest` **7/7, 51 aserciones** + `LoyaltyPortalCentralTest` **4/4, 17 aserciones, 0 fallos**. `LoyaltyCustomerPortal` 13/13, 89 aserciones.
- **P09A y siguientes: PENDIENTES.** Siguiente **P09A** – Código público único del cliente sin exponer ID interno/cédula/teléfono; no P09B QR+Code128, no P09C lectura POS, no P09D PIN/QR temporal.
