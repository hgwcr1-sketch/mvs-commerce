...
- **Evidencia P01–P09B:** `LoyaltyPortalSelfRegistrationTest` **11/11, 52 aserciones** + `LoyaltyPortalClientAccessTest` **11/11, 55 aserciones** + `LoyaltyPortalDeliveryTest` **7/7, 51 aserciones** + `LoyaltyPortalCentralTest` **4/4, 17 aserciones** + `CustomerPublicCodeTest` **5/5, 23 aserciones** + `CustomerQrBarcodeTest` **4/4, 16 aserciones, 0 fallos**. `LoyaltyCustomerPortal` 13/13, 89 aserciones.
- **P09C y siguientes: PENDIENTES.** Siguiente **P09C** – Escaneo QR/Code128 para seleccionar cliente en POS; no P09D PIN/QR temporal.
