...
| F33 | 10. Portal | QR | **PENDIENTE (BLOQUEADA)** | ALTA | F30,F34 | QR funciona y no expone información indebida | El stack actual NO tiene librería de QR (verificado en `composer.json`: solo dompdf/phpspreadsheet/tinker) y no se instalaron dependencias sin autorización. Arquitectura lista: `LoyaltyPortalAccessService::qrSupported()` devuelve false, el enlace seguro ya existe (`loyalty.portal.access`) y la pantalla "Accesos al portal" indica que el QR se habilitará con la generación local. Propuesta mínima cuando el usuario autorice: `chillerlan/php-qrcode` (PHP puro, genera PNG/SVG localmente, sin APIs externas que reciban el token). Prohibido usar servicios QR externos: enviarían el token del cliente a terceros |
...
| 10. Portal cliente | Web + QR + promociones | EN CURSO — F30–F32 y F34 COMPLETADOS; F33 bloqueada (dependencia QR sin autorizar); pendiente F35 |
...
- **Completadas:** F01–F32 y F34 según maestro (F28 completada de forma adelantada durante la integración POS, `7be1f80`).
...
- **F33 — QR: PENDIENTE/BLOQUEADA.** El stack no tiene librería QR y no se instaló ninguna sin autorización (regla del proyecto). Arquitectura preparada (`qrSupported()`, pantalla lista, enlace seguro ya funcional). Propuesta pendiente de aprobación del usuario: `chillerlan/php-qrcode` (PHP puro, local, sin APIs externas). Queda PROHIBIDO usar servicios QR externos que reciban el token.
- **SIGUIENTE:** **F35 — Publicidad/promociones** (etapa 10). F33 requiere decisión previa del usuario sobre la dependencia QR.
