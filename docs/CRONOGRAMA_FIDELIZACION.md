...
- **Completadas:** F01–F21 según maestro; F28 completada de forma adelantada durante la integración POS (`7be1f80`).
- **F21 — Historial de canjes: COMPLETADO.** Tabla `loyalty_reward_redemptions` (empresa, sucursal origen, cliente, premio, producto, usuario, puntos consumidos, movimiento vinculado, `event_key`, snapshots nombre/tipo/modo). Flujo atómico en `LoyaltyRewardRedemptionService`: validación empresa/activo → disponibilidad (`LoyaltyRewardAvailabilityService`) → cupo `limited` con `lockForUpdate` y descuento exacto de 1 unidad → historial → inventario exclusivamente vía `InventoryPostingService::postRewardRedemption` (tipo `reward_redemption`) → puntos vía `LoyaltyAccountService` con `TYPE_REWARD`. Idempotencia por `(company_id, event_key)` a nivel historial y movimientos. Interfaz "Canjes de premios" bajo permiso `fidelidad.canjes`.
- **SIGUIENTE:** **F22 — Vencimiento configurable** (etapa 6). Única fase autorizada para iniciar.
- **F29 — Ajuste por devolución:** PENDIENTE. Existe una brecha técnica conocida (`SaleReturnService` no ajusta fidelización en devoluciones parciales; la anulación completa sí revierte). Mantener registrada, pero **NO es la siguiente fase** y no debe adelantarse sobre F22–F27.
- Auditoría de referencia: regresión Loyalty en verde tras F21 (128 tests) más POS-Loyalty (48 tests); canjes: 11 tests, 61 aserciones.
