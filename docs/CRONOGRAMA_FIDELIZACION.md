...
- **Completadas:** F01–F22 según maestro; F28 completada de forma adelantada durante la integración POS (`7be1f80`).
- **F22 — Vencimiento configurable: COMPLETADO.** El administrador define Sí/No y una cantidad libre de meses enteros de inactividad (1–120, sin opciones rígidas). Campos `expiration_enabled`/`expiration_months` reutilizados de la infraestructura base; validación condicional en `UpdateLoyaltySettingRequest`; desactivar limpia los meses (representa claramente que no vencen). F22 solo configura la política: no vence puntos ni ejecuta procesos.
- **SIGUIENTE:** **F23 — Vencimiento automático** (etapa 6). Única fase autorizada para iniciar. Debe: respetar la política F22 (`expiration_enabled` + `expiration_months`), calcular inactividad sobre compras calificables (`last_qualifying_purchase_at`), generar salida de puntos trazable en Kardex con tipo `expiration` vía `LoyaltyAccountService`, idempotencia por cliente/período y ejecución controlada (command/job) — sin implementarlo aún en esta fase.
- **F29 — Ajuste por devolución:** PENDIENTE. Existe una brecha técnica conocida (`SaleReturnService` no ajusta fidelización en devoluciones parciales; la anulación completa sí revierte). Mantener registrada, pero **NO es la siguiente fase** y no debe adelantarse sobre F23–F27.
- Auditoría de referencia: regresión Loyalty en verde tras F22 (134 tests) más POS-Loyalty (48 tests); configuración de vencimiento: 7 tests, 62 aserciones.
