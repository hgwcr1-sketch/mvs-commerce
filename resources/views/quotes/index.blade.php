...
<div class="space-y-5">
    ...
    <x-card>
        ...
        @forelse($quotes as $quote)<tr><td class="p-3 font-semibold">{{ $quote->quote_number }}</td><td class="p-3">{{ $quote->created_at->format('d/m/Y H:i') }}</td><td class="p-3">{{ $quote->customer?->name ?? 'Consumidor Final' }}</td><td class="p-3">{{ $quote->status_label }}</td><td class="p-3 text-right">₡{{ number_format((float)$quote->total, 2, ',', '.') }}</td><td class="p-3"><div class="flex flex-wrap justify-end gap-3"><a class="underline" href="{{ route('cotizaciones.show', $quote) }}">Ver</a><a class="underline" target="_blank" href="{{ route('cotizaciones.print', $quote) }}">Imprimir</a>
