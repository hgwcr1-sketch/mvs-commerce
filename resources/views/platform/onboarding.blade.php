...
<div class="mx-auto max-w-4xl space-y-5" data-commercial-onboarding>
    ...
    <form method="POST" action="{{ route('platform.companies.store') }}" class="space-y-6 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-7">@csrf
        ...
        <section><h2 class="font-bold">Contrato inicial</h2><div class="mt-4 grid grid-cols-1 gap-4 md:grid-cols-3">
            <label class="text-sm font-semibold">Plantilla comercial<select name="license_plan_id" class="mt-2 min-h-11 w-full rounded-xl border px-4"><option value="">Contrato personalizado</option>@foreach($licensePlans as $plan)<option value="{{ $plan->id }}">{{ $plan->name }} · {{ $plan->branch_limit ?? '∞' }} suc. / {{ $plan->user_limit ?? '∞' }} usuarios</option>@endforeach</select></label>
            ...
            <label class="text-sm font-semibold">Límite de sucursales (override)<input type="number" min="1" name="branch_limit" value="{{ old('branch_limit') }}" class="mt-2 min-h-11 w-full rounded-xl border px-4"></label>
            <label class="text-sm font-semibold">Límite de usuarios (override)<input type="number" min="1" name="user_limit" value="{{ old('user_limit') }}" class="mt-2 min-h-11 w-full rounded-xl border px-4"></label>
