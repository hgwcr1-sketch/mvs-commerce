    ...
    @can('apartados.abonar')
        @if($layaway->status==='active'&&!$layaway->expires_at->isBefore(today()))
            <x-card>
                ...
                <form method="POST" action="{{route('apartados.payments.store',$layaway)}}" class="grid gap-3 md:grid-cols-5">
                    @csrf
                    <input required type="number" min="1" max="{{$layaway->balance_due}}" step="1" name="amount" placeholder="Monto (₡)"
                    ...
                    <select required name="payment_method_id" x-model="methodId" class="w-full rounded-lg border border-primary/70 px-3 py-2 focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/30">
                    ...
                    <input name="reference" placeholder="Referencia" class="w-full rounded-lg border border-primary/70 px-3 py-2 focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/30">
                    <input name="payment_notes" placeholder="Notas" class="w-full rounded-lg border border-primary/70 px-3 py-2 focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/30">
                    <input x-show="selectedMethod?.allows_change" x-cloak type="number" min="1" step="1" inputmode="numeric" name="received_amount" x-model="receivedAmount" placeholder="Monto recibido (efectivo)"
                           class="w-full rounded-lg border border-primary/70 px-3 py-2 text-right font-semibold focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/30">
                    <div x-show="selectedMethod?.allows_change" x-cloak class="md:col-span-4">
                    ...
                    <button class="min-h-11 rounded-lg bg-primary px-4 py-2 font-semibold text-slate-950 hover:bg-primary-hover md:col-span-5 md:w-fit">Registrar abono</button>
                </form>
                        ...
                        <tr class="border-t">
                            ...
                            <td class="p-2 text-center">
                                <button type="button" @click="printPaymentReceipt('{{route('mvs.print.ticket.layaway.payment',['layaway'=>$layaway->id,'payment'=>$p->id],false)}}', {{$p->id}})" :disabled="busyPrint"
                                        class="min-h-11 rounded-lg border border-primary/60 px-3 py-1.5 text-xs font-semibold text-[#806817] hover:bg-primary/10 disabled:opacity-50">
                                    Imprimir
                                </button>
                            </td>
                        </tr>
        ...
        @can('apartados.entregar')
            @if($layaway->status==='paid')
                <form method="POST" action="{{route('apartados.deliver',$layaway)}}">
                    @csrf
                    <button class="min-h-11 rounded-lg bg-emerald-600 px-4 py-2 font-semibold text-white hover:bg-emerald-700">Entregar y completar venta</button>
                </form>
            @endif
        @endcan
        @can('apartados.cancelar')
            @if(in_array($layaway->status,['active','paid']))
                <form method="POST" action="{{route('apartados.cancel',$layaway)}}" class="flex gap-2">
                    ...
                    <button class="min-h-11 rounded-lg bg-red-600 px-4 py-2 font-semibold text-white hover:bg-red-700">Cancelar</button>
                </form>
            @endif
        @endcan
        @if($layaway->sale)
            <a href="{{route('ventas.show',$layaway->sale)}}" class="min-h-11 inline-flex items-center rounded-lg bg-slate-900 px-4 py-2 text-white hover:bg-slate-800">Ver venta</a>
        @endif
    </div>
