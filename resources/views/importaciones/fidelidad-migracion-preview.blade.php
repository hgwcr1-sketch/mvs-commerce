...
@php
    ...
    $consolidatedCount = collect($rows)->sum(fn ($row) => max(0, (int) ($row['consolidated_count'] ?? 1) - 1));
...
<div class="mx-auto max-w-7xl space-y-6" data-loyalty-migration-preview>
    ...
    <div class="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <div class="rounded-xl border border-emerald-200 bg-emerald-50 p-4 text-sm text-emerald-800">Clientes listos<strong class="block text-2xl">{{ $validCount }}</strong></div>
        <div class="rounded-xl border border-sky-200 bg-sky-50 p-4 text-sm text-sky-800">Filas consolidadas<strong class="block text-2xl">{{ $consolidatedCount }}</strong></div>
        ...
        <section class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm"><div class="overflow-x-auto"><table class="w-full min-w-[1100px] divide-y divide-slate-200 text-sm"><thead class="bg-slate-50 text-left text-xs font-semibold uppercase text-slate-600"><tr><th class="px-4 py-3">Fila</th><th class="px-4 py-3">Nombre / cliente</th><th class="px-4 py-3 text-right">Otorgados</th><th class="px-4 py-3 text-right">Utilizados</th><th class="px-4 py-3 text-right">Saldo</th><th class="px-4 py-3">Estado</th></tr></thead><tbody class="divide-y divide-slate-100">
            @foreach($rows as $row)
                <tr class="align-top {{ $row['valid'] ? '' : 'bg-red-50/50' }}">
                    <td class="px-4 py-3">{{ implode(', ', $row['source_row_numbers'] ?? [$row['row_number']]) }}</td>
                    ...
                    <td class="px-4 py-3">@if($row['valid'])<span class="rounded-full bg-emerald-100 px-3 py-1 text-xs font-semibold text-emerald-700">Lista</span>@if(($row['consolidated_count'] ?? 1) > 1)<p class="mt-2 text-xs text-sky-700">{{ $row['consolidated_count'] }} filas consolidadas.</p>@endif @else<span class="rounded-full bg-red-100 px-3 py-1 text-xs font-semibold text-red-700">Pendiente</span><ul class="mt-2 space-y-1 text-xs text-red-700">@foreach($row['errors'] as $error)<li><strong>{{ $error['field'] }}:</strong> {{ $error['message'] }}</li>@endforeach</ul>@endif</td>
                </tr>
            @endforeach
    ...
    <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><p class="text-sm text-slate-600">La confirmación revalida e importa solo los clientes listos. Los pendientes se excluyen y quedan trazados con su motivo.</p>@if($validCount > 0)<form action="{{ route('importaciones.fidelidad-migracion.import') }}" method="POST" onsubmit="return confirm('¿Confirmar la migración P37 de los clientes listos?');">@csrf<button class="inline-flex min-h-11 w-full items-center justify-center rounded-xl bg-emerald-700 px-5 py-3 text-sm font-bold text-white sm:w-auto">Importar {{ $validCount }} clientes listos</button></form>@else<p class="rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm font-semibold text-red-700">No hay clientes válidos para importar.</p>@endif</div>
