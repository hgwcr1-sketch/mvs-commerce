...
<div class="mx-auto max-w-5xl space-y-6">
    ...
    @else
        ...
        @if($cashSession->accepts_usd_snapshot)
            <x-card><x-slot:header><h3 class="font-semibold">Efectivo en dólares</h3></x-slot:header>
                <dl class="grid gap-4 sm:grid-cols-3">
                    @foreach(['Esperado US$' => $cashSession->expected_cash_usd, 'Contado US$' => $cashSession->counted_cash_usd, 'Diferencia US$' => $cashSession->difference_amount_usd] as $label => $value)
                        <div class="rounded-xl border border-slate-400 p-4"><dt>{{ $label }}</dt><dd class="break-words font-semibold">{{ $value ?? 'Pendiente' }}</dd></div>
                    @endforeach
                </dl>
            </x-card>
        @endif
