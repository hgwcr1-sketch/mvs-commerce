...
@section('content')
<div class="mx-auto max-w-4xl" x-data="cashOpening()"><div class="mb-6 flex items-center justify-between"><div><h2 class="text-2xl font-semibold">Abrir caja</h2><p class="text-sm text-slate-600">Registra el fondo inicial contado.</p></div><a href="{{ route('cash.index') }}" class="inline-flex min-h-11 items-center rounded-lg border border-slate-300 px-4 py-2">Volver</a></div>
...
<input id="opening-denomination-{{ $denomination->id }}" name="denominations[{{ $denomination->id }}]" x-model="quantities[{{ $denomination->id }}]" type="number" inputmode="numeric" min="0" max="1000000" step="1" autocomplete="off" required class="min-h-11 w-full min-w-0 rounded-xl border-slate-300 px-3 py-2 text-right">
...
<div class="sticky bottom-20 z-10 mt-8 flex flex-wrap justify-end gap-3 rounded-xl bg-white p-3 shadow-sm"><a href="{{ route('cash.index') }}" class="rounded-xl border border-slate-300 px-6 py-3">Cancelar</a><button :disabled="processing" class="rounded-xl bg-amber-500 px-6 py-3 font-bold text-white hover:bg-amber-600 disabled:opacity-50" x-text="processing?'Abriendo…':'Abrir caja'"></button></div></form></x-card>@endif</div>
<script>
