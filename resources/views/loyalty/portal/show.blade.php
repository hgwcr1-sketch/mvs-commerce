...
<div class="space-y-6">
    ...
    @if($module_active)
        ...
        @if($posts->isNotEmpty())
            <section aria-label="Novedades" class="space-y-3"><h2 class="text-lg font-semibold">Novedades y avisos</h2><div class="grid grid-cols-1 gap-3 sm:grid-cols-2">@foreach($posts as $post)<article class="rounded-xl border bg-white p-4 {{ $post->is_featured ? 'ring-2 ring-amber-400' : '' }}"><p class="text-xs font-bold uppercase text-amber-700">{{ ['new_product' => 'Nuevo producto', 'offer' => 'Oferta', 'promotion' => 'Promoción', 'notice' => 'Aviso'][$post->type] }}</p><h3 class="mt-1 font-semibold">{{ $post->product?->name ?? $post->title }}</h3>@if($post->message)<p class="mt-2 text-sm text-slate-600">{{ $post->message }}</p>@endif @if($post->product)<p class="mt-2 font-semibold">{{ $money($post->product->special_price ?? $post->product->sale_price) }}</p>@endif</article>@endforeach</div></section>
        @endif
