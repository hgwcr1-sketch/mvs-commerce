...
<div class="space-y-6">
    ...
    <x-card>
        ...
        <form method="POST" action="{{ route('loyalty.registration-incentive.update') }}" class="max-w-xl space-y-4">
            ...
            <div class="grid gap-3 sm:grid-cols-2">
                <div class="rounded-xl border border-slate-200 bg-slate-50 p-4"><input type="hidden" name="require_verified_phone" value="0"><label class="flex min-h-11 items-start gap-3"><input type="checkbox" name="require_verified_phone" value="1" @checked(old('require_verified_phone', $registrationIncentive->require_verified_phone)) class="mt-1 h-5 w-5 rounded border-slate-300 text-amber-500 focus:ring-amber-400"><span><span class="block font-semibold text-slate-800">Exigir teléfono verificado</span><span class="block text-sm text-slate-500">No se crea la concesión hasta que el cliente tenga verificación registrada.</span></span></label></div>
                <div class="rounded-xl border border-slate-200 bg-slate-50 p-4"><input type="hidden" name="require_verified_email" value="0"><label class="flex min-h-11 items-start gap-3"><input type="checkbox" name="require_verified_email" value="1" @checked(old('require_verified_email', $registrationIncentive->require_verified_email)) class="mt-1 h-5 w-5 rounded border-slate-300 text-amber-500 focus:ring-amber-400"><span><span class="block font-semibold text-slate-800">Exigir correo verificado</span><span class="block text-sm text-slate-500">No se crea la concesión hasta que el cliente tenga verificación registrada.</span></span></label></div>
            </div>
