...
<header class="flex flex-col gap-3 md:flex-row md:items-start md:justify-between"><div><p class="text-sm font-semibold text-[#B1922D]">Compra {{ $verification->purchase->number }}</p><h1 class="text-2xl font-bold">Verificación digital</h1><p class="text-sm text-slate-600">{{ $verification->purchase->supplier?->commercial_name ?: $verification->purchase->supplier?->name }}</p></div><span class="self-start rounded-full bg-amber-100 px-3 py-2 text-sm font-bold text-[#B1922D]">{{ $statusLabels[$verification->status] }}</span></header>
