...
class CashSessionController extends Controller
{
    public function required(Request $request): View|RedirectResponse
    {
        ...
        if (app(CashSessionResolver::class)->applicable($request->user(), $companyId, $branchId)->isNotEmpty()) {
    ...
    public function create(Request $request): View|RedirectResponse
    {
        ...
        $denominations = CashDenomination::forCompany($companyId)->forCurrency('CRC')->active()->orderBy('sort_order')->get();
        $selectedRegisterId = ($registers->count() === 1 || $firstRegister?->is_default)
            ? $firstRegister?->id
            : null;

        return view('cash.open', ['denominations' => $denominations, 'registers' => $registers, 'selectedRegisterId' => $selectedRegisterId, 'settings' => $settings, 'cashier' => $request->user(), 'canManageRegisters' => $request->user()->hasPermission('caja.administrar', $company)]);
    }
...
return redirect()->route('cash.index')->with('success', "Caja {$session->session_number} abierta correctamente.");
    ...
    private function context(): array
    {
        ...
        $settings = CompanyCashSetting::where('company_id', $companyId)->firstOrFail();
