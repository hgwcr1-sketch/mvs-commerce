...
class CashClosingController extends Controller
{
    ...
    public function create(Request $request, CashSession $cashSession, CashExpectedAmountService $cashExpected, CashPaymentExpectedAmountService $paymentExpected, CashDenominationProvisioner $denominationProvisioner): View
    {
        ...
        $this->authorizeContinuation($request, $cashSession, $settings);
        ...
        $admin = $request->user()->hasPermission('caja.administrar', $company)
            || $request->user()->hasPermission('caja.ver_todas', $company);
        $blind = ! $admin || (bool) $cashSession->blind_closing_snapshot;
        $expectedBreakdown = $blind ? collect() : $paymentExpected->breakdown($cashSession);
        ...
        return view('cash.closing.create', [
            ...
            'expectedMethods' => $blind ? collect() : $paymentExpected->expectedAmounts($cashSession), 'expectedBreakdown' => $expectedBreakdown, 'requestToken' => (string) Str::uuid(),
        ]);
    }
    ...
    public function show(Request $request, CashSession $cashSession): View
    {
        ...
        if ($admin) {
            $cashSession->load(['branch:id,name', 'events.user:id,name', 'cashRegister:id,name', 'openedBy:id,name', 'closingStartedBy:id,name', 'closedBy:id,name', 'differenceAuthorizedBy:id,name', 'countDetails.cashDenomination', 'countDetails.countedBy:id,name', 'paymentReconciliations.reconciledBy:id,name']);
        }
        ...
        return view('cash.closing.show', [
            ...
            'closingSummary' => $admin ? app(CashClosingSummaryService::class)->summarize($cashSession) : null,
            'documentsCount' => $cashSession->documents_count,
        ]);
    }
    ...
    {
        [$company] = $this->context($request, $cashSession);
        abort_unless($request->user()->hasPermission('caja.autorizar_diferencia', $company), 403);
        abort_unless($cashSession->status === CashSession::STATUS_CLOSING && $cashSession->closing_submitted_at !== null, 409);
    ...
    {
        return [(int) session('active_company_id'), (int) session('active_branch_id')];
    }
    ...
    private function authorizeContinuation(Request $request, CashSession $session, CompanyCashSetting $settings): void
    {
        ...
        if ($settings->session_mode === CompanyCashSetting::SESSION_MODE_INDIVIDUAL) {
            abort_unless($session->opened_by === $request->user()->id, 403);
        } else {
            abort_unless($session->closing_started_by === $request->user()->id || $request->user()->hasPermission('caja.administrar', $session->company), 403);
        }
    }
