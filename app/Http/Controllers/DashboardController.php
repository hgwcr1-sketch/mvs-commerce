...
class DashboardController extends Controller
{
    public function index()
    {
        ...
        $base = AccountReceivable::query()->forCompany($company->id)->forBranch($branchId)->whereNotIn('status',[AccountReceivable::STATUS_PAID,AccountReceivable::STATUS_CANCELLED])->where('balance_due','>',0);
        $creditSummary = [
        ...
        if(auth()->user()->hasPermission('cuentas_pagar.ver',$company)){
            $row=AccountPayable::query()->forCompany($company->id)->forBranch($branchId)->whereNotIn('status',[AccountPayable::STATUS_PAID,AccountPayable::STATUS_CANCELLED])->where('balance_due','>',0)
