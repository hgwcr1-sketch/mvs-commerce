...
{
    public function index(Request $request, AdministrativeDashboardService $dashboard)
    {
        ...
        if (! $branchId) {
            ...
            if (auth()->user()->hasPermission('cuentas_pagar.ver', $company)) {
                ...
                foreach (['pending_count', 'overdue_count', 'upcoming_count'] as $key) {
                    $payableSummary[$key] = (int) $row->{$key};
                }
                foreach (['pending_amount', 'overdue_amount', 'upcoming_amount'] as $key) {
                    $payableSummary[$key] = (float) $row->{$key};
                }
            }

            if (auth()->user()->hasPermission('dashboard.admin', $company)) {

                return view('dashboard.index', compact('creditSummary', 'layawaySummary', 'payableSummary', 'dashboardSummary'));
