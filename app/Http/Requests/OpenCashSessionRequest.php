...
class OpenCashSessionRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        ...
        $settings = CompanyCashSetting::where('company_id', $companyId)->first();
        $accepts = (bool) $settings?->accepts_usd;
        $rate = ['nullable', 'numeric', 'gt:0'];
        if ($accepts) {
            $rate[0] = 'required';
            if ($settings->usd_exchange_rate_min !== null) {
                $rate[] = 'gte:'.$settings->usd_exchange_rate_min;
            } if ($settings->usd_exchange_rate_max !== null) {
                $rate[] = 'lte:'.$settings->usd_exchange_rate_max;
            }
        }
        ...
        return [
            ...
            'opening_amount' => ['nullable', 'numeric', 'min:0', 'regex:/^\d+$/'],
            'denominations' => ['required', 'array'],
            'denominations.*' => ['required', 'integer', 'min:0', 'max:1000000'],
