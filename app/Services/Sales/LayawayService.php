...
class LayawayService
{
    public function create(array $data, User $user, int $companyId, int $branchId): Layaway
    {
        ...
        $fingerprint = $this->fingerprint($data, $user->id, $companyId, $branchId);
        $clientToken = $data['client_token'] ?? null;

        if ($clientToken !== null) {
            $existing = $this->existingFromToken($companyId, $clientToken, $fingerprint);
            if ($existing !== null) {
                return $existing;
            }
        }
        ...
        try {
            return DB::transaction(function () use ($data, $user, $companyId, $branchId, $fingerprint, $clientToken) {
                $company = Company::findOrFail($companyId);
                $customer = Customer::forCompany($companyId)->where('is_active', true)->findOrFail($data['customer_id']);

                $canOverridePrice = $user->hasPermission('pos.cambiar_precio', $company);
                $total = 0;
                $lines = [];
                ...
                foreach ($data['items'] as $line) {
                    ...
                    $unit = (float) ($product->special_price ?? match ($customer->price_level ?? 'normal') {
                        'wholesale' => $product->wholesale_price !== null ? $product->wholesale_price : $product->sale_price,
                        'a' => $product->price_a !== null ? $product->price_a : $product->sale_price,
                        'b' => $product->price_b !== null ? $product->price_b : $product->sale_price,
                        'c' => $product->price_c !== null ? $product->price_c : $product->sale_price,
                        default => $product->sale_price,
                    });
                    if (array_key_exists('unit_price', $line) && $line['unit_price'] !== null && $line['unit_price'] !== '') {
                        if (! $canOverridePrice) {
                            throw ValidationException::withMessages(['items' => 'No tiene permiso para cambiar precios en el POS.']);
                        }

                        $unit = round((float) $line['unit_price'], 4);
                        if ($unit <= 0) {
                            throw ValidationException::withMessages(['items' => "El precio de {$product->name} debe ser mayor que cero."]);
                        }
                    }
                ...
                $layaway = Layaway::create([
                    ...
                    'created_by' => $user->id,
                    'number' => CompanySequence::nextLayawayNumber($companyId),
                    'client_token' => $clientToken,
                    'request_fingerprint' => $clientToken !== null ? $fingerprint : null,
                    'status' => Layaway::STATUS_ACTIVE,
                    'currency_code' => $company->currency ?? 'CRC',
                    'total' => $total,
                    'paid_total' => 0,
                    'balance_due' => $total,
                ...
                foreach ($lines as $line) {
                    ...
                    InventoryMovement::create([
                        ...
                        'quantity' => $line['qty'],
                        'previous_stock' => $line['stock']->stock,
                        'new_stock' => $new,
                        'reason' => 'Reserva por apartado',
                        'reference_type' => Layaway::class,
                        'reference_id' => $layaway->id,
                        'notes' => $layaway->number,
                    ]);
                }
            ...
            if ($clientToken !== null && $this->isUniqueConstraint($exception)) {
                return $this->existingFromToken($companyId, $clientToken, $fingerprint) ?? throw $exception;
            }
    ...
    private function fingerprint(array $data, int $userId, int $companyId, int $branchId): string
    {
        return hash('sha256', json_encode([
            ...
            'initial_amount' => isset($data['initial_amount']) ? number_format((float) $data['initial_amount'], 4, '.', '') : '0.0000',
            ...
            'items' => array_map(fn (array $item) => [
                ...
                'quantity' => isset($item['quantity']) ? number_format((float) $item['quantity'], 4, '.', '') : '0.0000',
                'unit_price' => isset($item['unit_price']) && $item['unit_price'] !== null && $item['unit_price'] !== ''
    ...
    private function existingFromToken(int $companyId, string $clientToken, string $fingerprint): ?Layaway
    {
        $existing = Layaway::query()
            ->where('company_id', $companyId)
        ...
        if (! hash_equals((string) $existing->request_fingerprint, $fingerprint)) {
            throw new ConflictHttpException('El token de apartado ya fue utilizado con datos diferentes.');
        }
