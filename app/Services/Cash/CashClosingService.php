...
class CashClosingService
{
    ...
    public function start(User $user, int $companyId, int $branchId, int $sessionId, string $token): CashSession
    {
        return DB::transaction(function () use ($user, $companyId, $branchId, $sessionId, $token) {
            ...
            if ($session->closing_request_token === $token && $session->status === CashSession::STATUS_CLOSING) {
                return $session;
            }
    ...
    public function submit(User $user, int $companyId, int $branchId, int $sessionId, array $data): array
    {
        return DB::transaction(function () use ($user, $companyId, $branchId, $sessionId, $data) {
            ...
            $denominations = CashDenomination::query()->forCompany($companyId)->forCurrency('CRC')->active()->orderBy('sort_order')->get();
            if ($denominations->isEmpty()) {
                throw ValidationException::withMessages(['denominations' => 'La empresa debe tener denominaciones CRC activas.']);
            }
            ...
            if ($submittedIds->all() !== $requiredIds->all()) {
                throw ValidationException::withMessages(['denominations' => 'Debe indicar exactamente todas las denominaciones CRC activas.']);
            }
            ...
            if ($submittedMethodIds->all() !== $requiredMethodIds->all()) {
                throw ValidationException::withMessages(['payments' => 'Debe reportar exactamente todas las formas de pago requeridas.']);
            }
            ...
            foreach ($denominations as $denomination) {
                ...
                $countedCash = bcadd($countedCash, $total, 4);
            ...
            $expectedCash = $this->cashExpected->calculateDecimal($session);
            ...
            foreach ($methods as $method) {
                ...
                CashPaymentReconciliation::create(['cash_session_id' => $session->id, 'payment_method_id' => $method->id, 'payment_method_code_snapshot' => $method->code, 'payment_method_name_snapshot' => $method->name, 'payment_method_type_snapshot' => $method->type, 'affects_cash_snapshot' => $method->affects_cash, 'sales_amount' => $breakdown['sales'], 'receivables_amount' => $breakdown['receivables'], 'layaways_amount' => $breakdown['layaways'], 'payables_amount' => $breakdown['payables'], 'expected_amount' => $expected, 'reported_amount' => $reported, 'difference_amount' => $difference, 'reference' => $data['payments'][$method->id]['reference'] ?? null, 'notes' => $data['payments'][$method->id]['notes'] ?? null, 'reconciled_by' => $user->id, 'reconciled_at' => $now]);
            }
    ...
    public function cancel(User $user, int $companyId, int $branchId, int $sessionId): CashSession
    {
        return DB::transaction(function () use ($user, $companyId, $branchId, $sessionId) {
            [$session, $settings] = $this->lockedContext($user, $companyId, $branchId, $sessionId, 'caja.cerrar');
            if ($session->status !== CashSession::STATUS_CLOSING || $session->closing_submitted_at !== null || $session->countDetails()->closing()->exists() || $session->paymentReconciliations()->exists()) {
                throw ValidationException::withMessages(['cash_session_id' => 'El cierre ya fue confirmado y no puede cancelarse.']);
            }
            $this->assertCanOperate($session, $settings, $user, true);
            $now = now();
    ...
    public function authorize(User $user, int $companyId, int $branchId, int $sessionId): CashSession
    {
        return DB::transaction(function () use ($user, $companyId, $branchId, $sessionId) {
            ...
            if ($session->status === CashSession::STATUS_CLOSED && $session->difference_authorized_by !== null) {
                return $session;
            }
            if ($session->status !== CashSession::STATUS_CLOSING || $session->closing_submitted_at === null) {
                throw ValidationException::withMessages(['cash_session_id' => 'No existe un cierre pendiente de autorización.']);
            }
            $now = now();
            $session->update(['difference_authorized_by' => $user->id, 'difference_authorized_at' => $now, 'status' => CashSession::STATUS_CLOSED, 'open_guard' => null, 'closed_by' => $user->id, 'closed_at' => $now]);
    ...
    private function lockedContext(User $user, int $companyId, int $branchId, int $sessionId, string $permission): array
    {
        ...
        if (! $company || ! $branch || ! $user->companies()->whereKey($companyId)->exists() || ! $user->branches()->whereKey($branchId)->exists() || ! $user->hasPermission($permission, $company)) {
        ...
        if (! $session || ! $session->cashRegister()->where('company_id', $companyId)->where('branch_id', $branchId)->where('is_active', true)->exists()) {
            throw ValidationException::withMessages(['cash_session_id' => 'La sesión o caja no está disponible.']);
        }
    ...
    {
        if ($settings->session_mode === CompanyCashSetting::SESSION_MODE_INDIVIDUAL && $session->opened_by !== $user->id) {
            throw ValidationException::withMessages(['cash_session_id' => 'Solo el cajero que abrió la sesión puede cerrarla.']);
        }
        if ($continuing && $settings->session_mode === CompanyCashSetting::SESSION_MODE_SHARED && $session->closing_started_by !== $user->id && ! $user->hasPermission('caja.administrar', $session->company)) {
            throw ValidationException::withMessages(['cash_session_id' => 'Solo quien inició el cierre o un administrador puede continuarlo.']);
        }
    }
